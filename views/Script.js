function getProducts(){
    firebase.firestore().collection("Products").get().then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
            document.getElementById('products').innerHTML += `<a class="card btn btn-outline-secondary mx-1" href="products/` + doc._key.path.segments[6] + `" style="width: 16rem; height: 25rem">
